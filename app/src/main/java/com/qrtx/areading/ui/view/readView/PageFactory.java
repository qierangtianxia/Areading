...
public class PageFactory {
    ...
    private List<Book.Chapter> chapterList;
    ...
    public int openBook(int chapter, int[] position) {
        ...
        this.chapterSize = chapterList.size();
    ...
    public synchronized void onDraw(Canvas canvas) {
        ...
        if (m_lines.size() > 0) {
            ...
            // 绘制标题
//            canvas.drawText(chapterList.get(currentChapter >= chapterList.size() ? chapterList.size() - 1 : currentChapter).title, marginWidth, y, mTitlePaint);
            canvas.drawText(chapterList.get(currentChapter - 1).title, marginWidth, y, mTitlePaint);
            ...
            if (batteryBitmap != null) {
                canvas.drawBitmap(batteryBitmap, marginWidth + ScreenUtils.dpToPxInt(2),
            ...
            if (listener != null) {
                listener.onSaveReadProgress(bookId, currentChapter, m_mbBufBeginPos, m_mbBufEndPos);
            }
            // 保存阅读进度
...
//        }
    ...
    public boolean hasNextPage() {
        return currentChapter < chapterList.size() || m_mbBufEndPos < m_mpBufferLen;
    }
